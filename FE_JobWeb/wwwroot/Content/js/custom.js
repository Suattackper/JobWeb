...
//  Client logo Carousel function by = owl.carousel.js ========================== //
	...
	function initializeDropzone() {
		if (jQuery('#resume-upload').length) {
			var dropzone = new Dropzone('#resume-upload', {
				url: "/your-upload-endpoint", // Thay bằng URL endpoint của bạn nếu có
				maxFilesize: 1, // Giới hạn kích thước file (MB)
				acceptedFiles: ".pdf,.doc,.docx", // Các loại file được chấp nhận
				parallelUploads: 2,
				thumbnailHeight: 120,
				thumbnailWidth: 120,
				autoProcessQueue: true, // Upload tự động
	
				init: function() {
					this.on("addedfile", function(file) {
						// Kiểm tra nếu có file cũ thì xóa file đó
						if (this.files.length > 1) {
							this.removeFile(this.files[0]); // Xóa file đầu tiên (file cũ)
						}
